...
function createCondicionSection(): HTMLElement {
  const section = document.createElement('div')
  section.className = 'flex flex-col gap-2'
...
function createEstadoSelect(): HTMLElement {
  const section = document.createElement('div')
  section.className = 'flex flex-col gap-2'

  const label = document.createElement('p')
  label.textContent = '¿En qué estado de Venezuela la recibiste? (Opcional)'
  label.className = 'text-xs md:text-sm font-medium text-stone-700'
...
export function createSurveyForm(
  ...
  submitBtn.addEventListener('click', () => {
    ...
    const condicionText =
      container.querySelector<HTMLElement>('.condicion-list [data-selected]')
        ?.childNodes[1]?.textContent ?? null
    const estadoSelect =
      container.querySelector<HTMLSelectElement>('#estado-select')
    const fileInput =
      container.querySelector<HTMLInputElement>('#photo-input')
    const data: SurveyData = {
      category: categoryBtn?.childNodes[1]?.textContent?.trim() ?? null,
      condicion: condicionText?.trim() ?? null,
      estado: estadoSelect?.value || null,
